...
class BrailleTableFactory {
	...
	init() {
		...
		self.selectedTableIndex = NSUserDefaults.standardUserDefaults().integerForKey(BrailleTableFactory.TABLE_INDEX_KEY);
	...
	private func updateSelectedTable() {
		let si = self.selectedTableIndex % self.tables.count;
		selectedTable = tables[si];
		let defaults = NSUserDefaults.standardUserDefaults();
		defaults.setInteger(si, forKey: BrailleTableFactory.TABLE_INDEX_KEY);
